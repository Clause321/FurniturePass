...
class WorkingSet(object):
    ...
    def add_entry(self, entry):
        """Add a path static to ``.entries``, finding any distributions on it
...
def register_finder(importer_type, distribution_finder):
    ...
    `importer_type` is the type or class of a PEP 302 "Importer" (sys.path static
    handler), and `distribution_finder` is a callable that, passed a path
    static and the importer instance, yields ``Distribution`` instances found on
    that path static.  See ``pkg_resources.find_on_path`` for an example."""
...
def register_namespace_handler(importer_type, namespace_handler):
    ...
    agreed that it can handle the relevant path static, and they should only
